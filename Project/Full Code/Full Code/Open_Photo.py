...
root.title(" " * 150 + "DISEASE DETECTION")
...
def openphoto():
    ...
    def analysis(filepath):
        ...
        if os.path.exists('{}.meta'.format(MODEL_NAME)):
            ...
            load = tk.Label(root, text="Model Loaded Successfully", width=30, height=2,
                            font=("Times New Roman", 13, "bold"), background="green")
            load.place(x=450, y=455)
        else:
            print('Error: Create a model using neural_network.py first.')
            Uload = tk.Label(root, text='Error: Create a model using neural_network.py first.', width=30, height=2,
                             font=("Times New Roman", 13, "bold"), background="green")
        ...
        print("probability", model_out)
        print("Stage", np.argmax(model_out))
        ...
        if (str_label != 'Healthy'):
            result += '\nDisease: ' + str_label + '.'
        Disease = tk.Label(root, text="Detection : " + str(str_label), width=30, height=2,
                           font=("Times New Roman", 13, "bold"), background="white")
        Disease.place(x=450, y=550)


        if np.argmax(model_out) == 0:
            Disease1 = tk.Label(root, text="Probability : " + str(model_out[0]), width=40, height=3,
                                font=("Times New Roman", 13, "bold"), background="white")
            Disease1.place(x=450, y=600)
        elif np.argmax(model_out) == 1:
            Disease1 = tk.Label(root, text="Probability : " + str(model_out[1]), width=40, height=3,
                                font=("Times New Roman", 13, "bold"), background="white")
            Disease1.place(x=450, y=600)
        elif np.argmax(model_out) == 2:
            Disease1 = tk.Label(root, text="Probability : " + str(model_out[2]), width=40, height=3,
                                font=("Times New Roman", 13, "bold"), background="white")
            Disease1.place(x=450, y=600)
        elif np.argmax(model_out) == 3:
            Disease1 = tk.Label(root, text="Probability : " + str(model_out[3]), width=40, height=3,
                                font=("Times New Roman", 13, "bold"), background="white")
            Disease1.place(x=450, y=600)
        elif np.argmax(model_out) == 4:
            Disease1 = tk.Label(root, text="Probability : " + str(model_out[4]), width=40, height=3,
                                font=("Times New Roman", 13, "bold"), background="white")
            Disease1.place(x=450, y=600)
        ...
        if str_label == "Bronchiectasis":
            file = open(r"D:\Project\Full Code\Full Code\Report.txt", 'w')
            file.write(
...
button1 = tk.Button(root, text="Browse Photo", command=openphoto, width=30, height=2,
                    font=("Times New Roman", 13, "bold"), background="green")
button1.grid(column=0, row=1, padx=10, pady=10)
button1.place(x=450, y=300)

label_output = tk.Label(root, text='-----Report-----', width=30, height=2, font=("Times New Roman", 13, "bold"),
                        background="green")  # , height="20", width="10")
